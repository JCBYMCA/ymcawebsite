...
const TemplatePage = (props) => {
    ...
    const [title, setTitle]= useState(urlKey[1]);
    const [menu, setMenu]= useState('');
    const [showArch, setShowArch]= useState(false);

    useEffect(() => {
        let user = urlKey?departmentId[urlKey[0]]:"1";
        // if(isDepartment){
        ...
        getNotices(user, urlKey[1]).then((resp) => {
            ...
            <div className={' bg-logo bg-center h-fit bg-no-repeat bg-auto rounded-md border-1 text-justify bg-neutral-50 border-neutral-300 flex shadow-2xl'}>
                <div className={" px-6 w-full py-8 bg-white/90 "}>
                    ...
                    <div className={'mr-4 w-full'}>
                        <h1 className={'font-bold text-2xl mb-4 text-center'}>{title==='scheme_syllabus'?'Scheme & Syllabus':'Notices'}</h1>
