    ...
    return (
        <div className={`fbox ${index%2===0 ? 'fbox-even' : 'fbox-odd'}`}>
            ...
            <div className={`section-b p-2 flex items-center flex-col text-justify`}>
                ...
                <h1 className={'text-xs font-thin mt-2 mb-12 '}>{t(description)}</h1>
                <button className={'text-xxs absolute bottom-44 mx-auto md:text-sm bg-white px-2 md:px-3 hover:scale-105 duration-200 text-black'}>{t("Read More")}</button>
            </div>
        </div>
    )
}
